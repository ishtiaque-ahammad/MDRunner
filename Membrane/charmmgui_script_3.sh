...
echo "Welcome to MD simulation result analysis! Firstly, We will convert .trr file to .xtc file. When prompted, select 0 (system) for output"

sleep 3s

gmx trjconv -f step7_production.trr -s step7_production.tpr -o step7_production.xtc

echo "Now, we will correct the trajectory of the simulation. When prompted, 
select 1 (Protein) as the group to be centered and 0 (System) for output"

sleep 3s

gmx trjconv -s step7_production.tpr -f step7_production.xtc -o step7_production_corrected.xtc -pbc mol -center
...
gmx rms -s step7_production.tpr -f step7_production_corrected.xtc -o rmsd.xvg -tu ns
...
gmx rmsf -s step7_production.tpr -f step7_production_corrected.xtc -o rmsf.xvg -res
...
gmx gyrate -s step7_production.tpr -f step7_production_corrected.xtc -o gyrate.xvg
echo "The radius of gyration graph has been generated. We will do SASA calculation now. 
When prompted, choose group 1 (Protein) for analysis."
sleep 3s

#sasa calculation
gmx sasa -s step7_production.tpr -f step7_production_corrected.xtc -o sasa.xvg
